...
The communication bridge depends on the `services` branch of `swarm_topology_bridge`. It is an independent repository placed next to `safe_valley_exp` under the workspace `src/` directory, so that catkin can discover it as a separate ROS package:
